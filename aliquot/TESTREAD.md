...
In order to execute the program, it needs to be compiled first by the command gcc -O0 -m32 -Wall -Wextra -Werror -pedantic -o aliquot aliquot.c in any Linux lab. Afterwards, the program can be executed smoothly and run as intended.
...
After printing out the messages that the exercise states and keeping the user's inputs into 3 variables,an if statement is created in order to print out the first number of the sequence in case the value of the "choice" variable was f. 
